...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	defer func() {
		...
		if !reply.Success {
			LOG(rf.me, rf.currentTerm, DDebug, "<- S%d, Follower Conflict: [%d]T%d", args.LeaderId, reply.ConfilictIndex, reply.ConfilictTerm)
			LOG(rf.me, rf.currentTerm, DDebug, "<- S%d, Follower Log=%v", args.LeaderId, rf.log.String())
		}
	}()
	// return failure if PrevLogIndex not match
...
func (rf *Raft) startReplication(term int) bool {
	replicateToPeer := func(peer int, args *AppendEntriesArgs) {
		...
		if !ok {
			LOG(rf.me, rf.currentTerm, DAppend, "-> S%d, Lost or crashed", peer)
			return
		}
	...
	if rf.contextLostLocked(Leader, term) {
		LOG(rf.me, rf.currentTerm, DAppend, "Lost Leader[T%d] to %s[T%d]", term, rf.role, rf.currentTerm)
		return false
	}
